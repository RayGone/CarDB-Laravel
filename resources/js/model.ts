...
export const filterAttributes = columnDef
    .filter((col) => col.key != 'id')
    .filter((col) => col.key != 'name')
    .filter((col) => col.key != 'origin');
...
export const isInitFilter = (f: DataFilterModel) => {
    if (
        JSON.stringify(f.filter) == JSON.stringify(initFilter.filter) &&
        f.limit == initFilter.limit &&
        f.orderBy == initFilter.orderBy &&
        f.order == initFilter.order &&
        f.limit == initFilter.limit &&
        f.page == initFilter.page &&
        f.search == initFilter.search
    )
        return true;
