...
export function AddFilter({onClose, onAdd}: {onAdd: (f: FilterModel)=>void, onClose: (e)=>void}){
    ...
    const [addFilterState, addFilterDispatcher] = useReducer<FilterModel, any>(filterAddReducer, initOps);
    ...
    return <div className='inline-flex items-center p-2 w-full'>
        <div className='flex flex-col w-5/6'>
            ...
            <select id="attribute" className='rounded p-2 mb-3 border text-sm ' value={addFilterState.field} aria-placeholder='Attribute' onChange={(e)=>{addFilterDispatcher({field: e.target.value})}}>
                <option value={'-1'} className="bg-white dark:bg-gray-800 font-thin">Select Attribute</option>
                {
                    filterAttributes.map((attr)=><option key={attr.key} value={attr.key} className="bg-white dark:bg-gray-800 ">
                        {attr.header}
                    </option>)
                }
            </select>
            ...
            <select id="ops" className='border rounded p-2 mb-3 text-sm ' aria-placeholder='Conditions' value={addFilterState.ops} onChange={(e)=>{addFilterDispatcher({ops: e.target.value})}}>
                <option value={'-1'} className="text-sm font-thin bg-white dark:bg-gray-800">Select Condition</option>
                {
                    filterOps.map((op)=><option key={op.key} value={op.key} className="bg-white dark:bg-gray-800 ">
                        {op.value}
                    </option>)
                }
            </select>
...
export default function FilterView(
    ...
    return <div className={"bg-white dark:bg-gray-950 shadow-md dark:shadow-gray-800 sm:rounded-lg p-2 flex flex-col min-w-64 " + className} {...props}>
        ...
        {
            openAddFilter && <>
                <AddFilter onClose={()=>{setOpenAddFilter(false)}} onAdd={(f)=>{onFilter(f); setOpenAddFilter(false);}} />
                <hr className='border-t-2' />
            </>
        }
