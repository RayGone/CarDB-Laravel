...
function formValidation(state: Car) {
    ...
    if (
        ...
        required.some((col) => {
            const status =
                (state[col.type] === 'number'
                    ? state[col.key] < 0
                    : state[col.key] === '') || state[col.key] === null;
...
export default function FormCar({
    ...
    const handleSubmit = (e: MouseEvent) => {
        ...
        if (!formValidation(data)) {
            ...
            errorMsg.current = (
                <span>
                    ...
                    <br />
                    [Name, Origin and Model Year] are required fields!!
                    <br /> Numeric values must not be less than 0.
                </span>
            );
            return;
        }
        ...
        const endpiont = !!car
            ? route('api.cars.edit', { id: data?.id ?? -1 })
            : route('api.cars.add');
        setPost(true);
    ...
    return (
        <div className="my-2 h-full w-full min-w-72 px-8 py-4 text-xs md:text-sm">
            ...
            <form
                className="my-2 flex w-full flex-col items-start justify-start space-y-2"
            ...
            >
                {columnDef.map(
                    (col) =>
                        ...
                        col.key != 'id' && (
                            <div
                                key={col.key}
                                className="flex w-full flex-col space-y-1 pb-3"
                            >
                                <label htmlFor={col.key}>{col.header}:</label>
                                <input
                                    id={col.key}
                                    key={col.key}
                                    className="h-10 w-full rounded border px-2 dark:border-slate-700"
                                    type={col.type}
                                    value={
                                        data[col.key] == -1
                                            ? null
                ...
                {isError && (
                    <span className="w-full rounded bg-red-100 p-4 text-red-500">
                        {errorMsg.current}
                    </span>
                )}
